import { Announcement, Priority } from '@/data/admin';
...
export function AnnouncementCard({ announcement, onDelete }: AnnouncementCardProps) {
  ...
  return (
    <PremiumCard className="p-4 border-l-4 border-l-gold-500">
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex items-center gap-2">
            <span className="text-[10px] text-text-secondary flex items-center gap-1">
                <Clock size={10} /> {new Date(announcement.startDate).toLocaleDateString()}
            </span>
            {onDelete && (
                <button 
      ...
      <p className="text-xs text-text-secondary leading-relaxed">{announcement.content}</p>
    </PremiumCard>
  );
}
